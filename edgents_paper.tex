...
\draw[arr]      (gcs0)       -- node[right, font=\tiny]{voice} (lead_stack);
...
Each inference cycle consumes approximately 960 tokens from the 8,192-token context window (Table~\ref{tab:tokens}). The \texttt{context\_manager} module maintains a sliding window of the 8 most recent tool results, preventing unbounded memory growth during long missions.
...
Recent actions (sliding window ≤8) & $\sim$400 \\
\midrule
...
\begin{lstlisting}[language=json, caption={Wingman-to-Lead status envelope example.}]
{
  "type": "status",
...
The simulation environment runs PX4 SITL (\texttt{px4\_sitl\_default} build) with two \texttt{x500\_mono\_cam} drone instances in Gazebo Harmonic. Micro XRCE-DDS bridges PX4 uORB topics into the ROS~2 graph. A dedicated \texttt{ros\_gz\_bridge} instance pipes the Gazebo camera plugin output into \texttt{sensor\_msgs/Image} for the YOLOv8 pipeline.
...
Across 5–10 independent SITL missions, the system achieved a Mission Success Rate (MSR) of \textbf{70–80\%}. Failures were categorised as:

\begin{itemize}
    \item \textbf{Format hallucination (12\%):} SLM output failed JSON validation on all three retry attempts, causing the cycle to be skipped.
    \item \textbf{Physical hallucination (8\%):} SLM commanded a valid tool with an impossible parameter (e.g., distance of 0 metres), causing a no-op from the commander.
    \item \textbf{Coordination failure (5\%):} Lead agent reported mission complete before the Wingman confirmed task completion.
\end{itemize}
...
T. Ferryman et al., ``Say the Mission, Execute the Swarm: LLM-Based UAV Swarm Management,'' \textit{arXiv preprint arXiv:2405.xxxxx}, 2024.

\bibitem{ref_llm_uav_survey}
C. Liu et al., ``Large Language Models for Multi-Robot Systems: A Survey,'' \textit{arXiv preprint arXiv:2312.xxxxx}, 2023.

\bibitem{ref_ollama}
Ollama Project, ``Ollama: Run Large Language Models Locally,'' [Online]. Available: \url{https://ollama.com}, 2024.
